...
const post = computed(() => splitMarkdown(parseMarkdown(props.markdown)));
...
<template>
  <div>
    <div v-for="segment in post" :key="segment.type">
      ...
      <photo-preview v-else-if="segment.type === 'PHOTO' && photoStore.byName(segment.data[0])" :photo="photoStore.byName(segment.data[0])"/>
    </div>
  </div>
</template>
